...
- **Сервисы на VPS:** N8N (n8n.marbomebel.ru), VK Long Poll listener, VK Callback receiver, Claude Chat PWA (chat.marbomebel.ru)

### GitHub Actions Workflows
- `deploy-p0.yml` — полный деплой на VPS (N8N + Long Poll + workflows)
- `deploy-claude-chat.yml` — деплой Claude Chat PWA на VPS (PM2 + Traefik)
...
├── claude-chat/           — Claude VPS Chat PWA (Express, PM2, Traefik)
├── diagnostics/           — авто-диагностика деплоев (коммитится из CI)
...
- [x] Claude Chat PWA (chat.marbomebel.ru) — задеплоен, HTTPS через Traefik, PM2
- [x] Traefik file provider настроен для динамической маршрутизации (chat.marbomebel.ru)
- [x] NGINX остановлен и отключён на VPS (Traefik управляет всем трафиком)
...
- **Claude Chat PWA** (chat.marbomebel.ru) — веб-панель управления VPS через Claude CLI, Express :3847, PM2
- **Traefik** — reverse proxy в Docker, управляет TLS (Let's Encrypt), маршрутизация n8n + chat через file provider
- **NGINX** — отключён (конфликтовал с Traefik на портах 80/443)
...
- Claude Chat PWA через Traefik file provider (не NGINX) — Traefik управляет всем TLS трафиком
- Самодиагностика деплоя: fix-proxy.sh → SCP диагностика → коммит в repo через GitHub Contents API
